...
bool isIncreaseChainNumber(long long number) {
    string numberString = to_string(number);
    for (int i = 0; i < numberString.length() - 2; i++) {
        if (numberString[i] >= numberString[i + 1] || numberString[i + 1] >= numberString[i + 2]) {
            return false;
        }
    }
}

long long number1 = 123456789;
long long number2 = 123432112321;
long long number3 = 988811111;

cout << isIncreaseChainNumber(number1) << endl;  // 1
cout << isIncreaseChainNumber(number2) << endl; // 1
